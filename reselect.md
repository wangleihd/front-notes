...
* 减少存储 Selectors can compute derived data, allowing Redux to store the minimal possible state.
* 减少计算 Selectors are efficient. A selector is not recomputed unless one of its arguments change.
* 可组合 Selectors are composable. They can be used as input to other selectors.
